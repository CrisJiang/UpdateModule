...
bool CUnzipper::UnzipTo(LPCTSTR szFolder, LPCTSTR szPwd)
{
	if (!m_uzFile)
		return FALSE;
...
bool CUnzipper::UnzipFile(LPCTSTR szFolder, LPCTSTR szPwd)
{
	if (!m_uzFile)
		return FALSE;
	...
	if (unzOpenCurrentFilePassword(m_uzFile, szPwd) != UNZ_OK)
	{
		CloseHandle(hOutputFile);
		return FALSE;
	}
